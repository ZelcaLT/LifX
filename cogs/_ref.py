...
async def html_ref(ctx, text):
    ...
    emb = nextcord.Embed(title=text, description=output, url=url)
...
async def _git_main_ref(part, ctx, text):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title=title, url=url)
...
async def sql_ref(ctx, text):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title=title, url=url, description=description)
...
async def haskell_ref(ctx, text):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title=title, description=description, url=url)
