...
from cogs._tio import Tio

class used(commands.Cog, name="used"):
...
def prepare_payload(payload):
    ...
    try:
        ...
        emb = nextcord.Embed(title='SyntaxError',description=f"Command `run` missing a required argument: `code`",colour=0xff0000)
        return ('', emb, True)

    return (language, text, False)

async def get_message(interaction: nextcord.Interaction, fetch=False) -> nextcord.Message:
    ...
    if not fetch:
        ...
        if isinstance(message, nextcord.DeletedReferencedMessage):
            return None
...
class RerunBtn(nextcord.ui.Button):
...
class Refresh(nextcord.ui.View):
    def __init__(self, bot, no_rerun, timeout=300):
        super().__init__()

        item = RerunBtn(bot=bot, label='Run again', style=nextcord.ButtonStyle.grey, emoji='🔄', disabled=no_rerun)

        self.add_item(item)

        self.children.reverse() # Run again first

    @nextcord.ui.button(label='Delete', style=nextcord.ButtonStyle.grey, emoji='🗑')
