...
class Documantion(commands.Cog, name="Docs"):
...
async def python_doc(ctx, text: str):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title="Python 3 docs")
...
async def _cppreference(language, ctx, text: str):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title=f"{language} docs")
...
async def haskell_doc(ctx, text: str):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            emb = nextcord.Embed(title='Haskell docs')
